...
module.exports.find = function (options) {
  ...
  if (options.toolsVersion === 'auto') {
    ...
    if (options.toolsVersion === 15.0 || options.toolsVersion === "Current") {
  ...
  if (version === '12.0' || version === '14.0' || version === '15.0' || version === 'Current') {
    var x64_dir = is64Bit ? 'amd64' : '';
    var msBuildPath = path.join(msbuildRoot, 'MSBuild', version, 'Bin', x64_dir, 'MSBuild.exe');
    console.log('MSBUILD1: '+ msBuildPath);
    
    if (fs.existsSync(msBuildPath)) {
      return msBuildPath;
    } else {
      msBuildPath = path.join(msbuildRoot, 'MSBuild', 'Current', 'Bin', x64_dir, 'MSBuild.exe');
      console.log('MSBUILD1: '+ msBuildPath);
    }
